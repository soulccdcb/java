...
RUN set -x && apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 648ACFD622F3D138  && apt-get update && apt-get install -y --no-install-recommends  openssh-server tzdata  && rm -rf /var/lib/apt/lists/* /etc/apt/sources.list.d/*
